...
function normalizeAssetPath(rawPath, assetExts, allowedDomains) {
    ...
    if (value.startsWith('http://') || value.startsWith('https://')) {
        if (!isAssetUrl(value, assetExts)) return null;
        if (allowedDomains && allowedDomains.size > 0) {
            try {
                const hostname = new URL(value).hostname;
                if (!allowedDomains.has(hostname)) return null;
            } catch {
                return null;
            }
        }
...
function extractAssetPathsFromText(text, { assetExts = [], loosePrefixes = [], allowedDomains } = {}) {
    ...
    for (const regex of patterns) {
        for (const match of text.matchAll(regex)) {
            const candidate = normalizeAssetPath(match[1] || match[0], assetExts, allowedDomains);
...
function extractAssetPathsFromJson(value, { assetExts = [], loosePrefixes = [], allowedDomains } = {}) {
    const output = new Set();
    ...
    function recurse(v) {
        ...
        const direct = normalizeAssetPath(v, assetExts, allowedDomains);
        if (direct) output.add(direct);

        for (const item of extractAssetPathsFromText(v, { assetExts, loosePrefixes, allowedDomains })) {
            output.add(item);
        }
    }
