...
const { createFileInventory, createMirrorManifest, writeMirrorManifest } = require('./manifest');
...
function rewriteTextForLocalMirror(text) {
  ...
  return rewriteExternalUrlsForLocalMirror(text)
    .replaceAll(CONFIG.targetHost, \`/\${MIRROR_NAME}\`)
...
function send(req, res, status, headers, data) {
  ...
  if (encoding && data && data.length > 1024 && COMPRESSIBLE_CT.has(contentType)) {
    data = encoding === 'gzip' ? zlib.gzipSync(data) : zlib.deflateSync(data);
...
function exportStandaloneProject({
    rootDir,
    config,
    ...
    writeMirrorManifest(path.join(mirrorDest, '.mirror-manifest.json'), createMirrorManifest({
        ...
        resources: [],
        pending: [],
        files: createFileInventory(mirrorDest),
