...
async function portCheck(config) {
    const available = await portAvailable(config.port);
    if (available) {
        return makeCheck('port', 'pass', `Port ${config.port} is available.`);
    }
    if (config.autoPort) {
        return makeCheck('port', 'warn', `Port ${config.port} is not available; auto-port is enabled and startup will try the next available port.`);
    }
    return makeCheck('port', 'fail', `Port ${config.port} is not available.`);
}
